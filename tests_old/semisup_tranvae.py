...
for experiment in experiments:
    for test_nr in test_nrs:
        ...
        if experiment == "lung_h":
            adata = sc.read(
                os.path.expanduser(
                    f"~/Documents/benchmarking_datasets/adata_lung_subsampled.h5ad"
                )
            )
            condition_key = "study"
        ...
        if unlabeled_strat == "ct":
            ...
            for celltype in cts:
                ...
                ct_sel_ind = np.random.choice(
                    ct_indices, size=cells_per_ct, replace=False
                )
        ...
        ref_path = os.path.expanduser(
            f"~/Documents/{save_dir}/{experiment}/{test_nr}_model"
        )
        tranvae.save(ref_path, overwrite=True)
        text_file_t = open(
            os.path.expanduser(
                f"~/Documents/{save_dir}/{experiment}/{test_nr}_runtime.txt"
            ),
            "w",
        )
        ...
        if len(unlabeled_adata) > 0:
            ...
            data_latent = tranvae.get_latent(
                unlabeled_adata.X, unlabeled_adata.obs[condition_key]
            )
            ...
            for cell_key in cell_type_key:
                ...
                n = text_file_q.write(
                    classification_report(
                        ...
                        y_pred=preds,
                        labels=np.array(
                            unlabeled_adata.obs[cell_key].unique().tolist()
                        ),
                    )
                )
                text_file_q.close()
                ...
                plt.savefig(
                    os.path.expanduser(
                        f"~/Documents/{save_dir}/{experiment}/{test_nr}_query_uncertainty_{cell_key}.png"
                    ),
                    bbox_inches="tight",
                )
            ...
            sc.pl.umap(
                adata_latent, color=["batch"], frameon=False, wspace=0.6, show=False
            )
            plt.savefig(
                os.path.expanduser(
        ...
        for cell_key in cell_type_key:
            ...
            text_file_f = open(
                os.path.expanduser(
                    f"~/Documents/{save_dir}/{experiment}/"
                    f"{test_nr}_full_acc_report_{cell_key}.txt"
                ),
                "w",
            )
            n = text_file_f.write(
                classification_report(y_true=adata.obs[cell_key], y_pred=preds)
            )
            text_file_f.close()
