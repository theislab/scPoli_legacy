    ...
    for test_nr in test_nrs:
        ...
        tranvae = TRANVAE.load_query_data(
            ...
            reference_model=os.path.expanduser(
                f"~/Documents/{save_dir}/{experiment}/{test_nr}_ref_model"
            ),
            labeled_indices=[],
        )
        q_time = time.time()
        ...
        tranvae.save(
            os.path.expanduser(f"~/Documents/{save_dir}/{experiment}/{test_nr}_model"),
            overwrite=True,
        )
        text_file_t = open(
            os.path.expanduser(
                f"~/Documents/{save_dir}/{experiment}/{test_nr}_query_runtime.txt"
            ),
            "w",
        )
        ...
        results_dict = tranvae.classify(
            adata.X, adata.obs[condition_key], metric=class_metric
        )
        for cell_key in cell_type_key:
            preds = results_dict[cell_key]["preds"]
            probs = results_dict[cell_key]["probs"]
            text_file_f = open(
                os.path.expanduser(
                    f"~/Documents/{save_dir}/{experiment}/{test_nr}_full_acc_report_{cell_key}.txt"
                ),
                "w",
            )
            n = text_file_f.write(
