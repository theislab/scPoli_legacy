...
parser.add_argument("--experiment", type=str, help="Dataset of test")
parser.add_argument("--testnr", type=int, help="Number of test", default=10)

parser.add_argument("--data_dir", type=str, help="Number of test", default="../data")
...
for experiment in experiments:
    for test_nr in test_nrs:
        ...
        if experiment == "pbmc":
            adata = sc.read(f"{DATA_DIR}/benchmark_pbmc_shrinked.h5ad")
        ...
        for i in range(len(cell_type_key)):
            ...
            text_file_q = open(
                f"{RESULTS_DIR}/{experiment}/{test_nr}_query_acc_report_{i}.txt", "w"
            )
            n = text_file_q.write(
                classification_report(
                    ...
                    y_pred=preds,
                    labels=np.array(
                        unlabeled_adata.obs[cell_type_key[i]].unique().tolist()
                    ),
                )
            )
            text_file_q.close()
            ...
            adata_latent.obs[cell_type_key[i]] = unlabeled_adata.obs[
                cell_type_key[i]
            ].tolist()
        ...
        for i in range(len(cell_type_key)):
            ...
            n = text_file_f.write(
                classification_report(y_true=adata.obs[cell_type_key[i]], y_pred=preds)
            )
            text_file_f.close()
